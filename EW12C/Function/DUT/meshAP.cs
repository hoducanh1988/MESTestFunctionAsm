...
namespace EW12C.Function.DUT {
    ...
    public class meshAP<T> : SSH<T> where T : class, new() {
        ...
        public string getWanSpeed() {
            ...
            //get data
            string data_out = this.Query("cat /sys/class/net/eth0/speed", 100);
            string[] buffer = data_out.Split('\n');
            if (buffer.Length < 2) return null;

            //split data
            string speed = buffer[1].Replace("\r", "").Replace("\n", "").ToUpper().Trim();
        ...
        public bool setLedWanBrightGreen() {
            ...
            //r = this.Query("echo 0 > /sys/class/leds/led_blue//brightness", "root@VNPT:~#", 1000, 3, out data_out);
            r = this.Query("echo 0 > /sys/class/leds/led_red/brightness", "root@VNPT:~#", 1000, 3, out data_out);
            return r;
        }

        public bool setLedWanBrightYellow() {
            if (!IsConnected) return false;
            ...
            //r = this.Query("echo 1 > /sys/class/leds/led_blue//brightness", "root@VNPT:~#", 1000, 3, out data_out);
            r = this.Query("echo 0 > /sys/class/leds/led_red/brightness", "root@VNPT:~#", 1000, 3, out data_out);
            return r;
        }
